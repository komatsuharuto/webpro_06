...
app.post("/delete", (req, res) => {
  ...
  if (index >= 0 && index < bbs.length) {
    ...
    res.json({ success: false, message: "無効なインデックスです。" });
  }
});
...
app.post("/edit", (req, res) => {
  ...
  if (index >= 0 && index < bbs.length && newMessage) {
    bbs[index].message = newMessage;
    res.json({ success: true, message: "メッセージを編集しました。" });
  } else {
    res.json({ success: false, message: "無効なデータです。" });
  }
});
...
app.post("/like", (req, res) => {
  ...
  if (index >= 0 && index < bbs.length) {
    ...
    res.json({ success: true, likes: bbs[index].likes });
  } else {
    res.json({ success: false, message: "無効なインデックスです。" });
  }
});
